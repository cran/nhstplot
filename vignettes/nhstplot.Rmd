...
First, you need to load the library for it to work, so:
...
The number of significant digits can be modified in all functions using `signifdigitsp` for the $p$ value, and `signifdigitschisq` / `signifdigitsf` / `signifdigitst` / `signifdigitsz` for the test statistic. The default is `3` for all.
